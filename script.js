...
function recordVisitData(name, url, iconHtml) {
...
function recordVisit(card) {
    ...
    recordVisitData(name, url, getCardIconHtml(card));
}
...
function attachCardEvents() {
    toolCards.forEach(function(card) {
        ...
        card.addEventListener('click', function(e) {
            // 如果点击的是收藏按钮或相关链接按钮，不记录访问
            if (e.target.closest('.tool-card-fav, .related-toggle, .related-menu-item')) return;
            recordVisit(card);
        });
    });
}
...
function saveFavTools(fav) {
    try { localStorage.setItem(FAV_KEY, JSON.stringify(fav)); } catch(e) {}
}

function isFavorited(url) {
    if (!url) return false;
    return getFavTools().some(function(item) { return item.url === url; });
}
...
function toggleFavorite(card) {
    ...
    if (!url) return;
    var name = card.querySelector('.tool-name')?.textContent || '';
    var iconHtml = getCardIconHtml(card);
    var fav = getFavTools();
    var idx = -1;
    fav.forEach(function(item, i) { if (item.url === url) idx = i; });
    if (idx > -1) {
        fav.splice(idx, 1);
    } else {
        fav.push({ name: name, url: url, iconHtml: iconHtml, time: Date.now() });
    }
    saveFavTools(fav);
    renderFavSection();
    renderFavState();
...
function renderFavState() {
    toolCards.forEach(function(card) {
        var url = card.getAttribute('href') || card.getAttribute('data-url') || '';
        var btn = card.querySelector('.tool-card-fav');
        if (btn) btn.classList.toggle('active', isFavorited(url));
    });
}
...
function initFavButtons() {
    toolCards.forEach(function(card) {
        // 避免重复添加
        if (card.querySelector('.tool-card-fav')) return;

        var btn = document.createElement('button');
        btn.className = 'tool-card-fav';
        btn.setAttribute('aria-label', '收藏');
        btn.title = '收藏';
        btn.innerHTML = '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>';
...
const PROGRESS_SPIN_DURATION = 6000; // 6秒一圈
const SNAKE_MIN_LENGTH = 8;
const FOOD_RADIUS = 2.2;
let snakeLength = 0;
let foods = [];
...
function getShiftProgress(now) {
    ...
    if (currentMinutes < MORNING_START) {
        ...
        return (currentMinutes + DAY_MINUTES - previousSchedule.dayEnd) / (MORNING_START + DAY_MINUTES - previousSchedule.dayEnd) * 100;
    }

    if (currentMinutes < MORNING_END) {
        return (currentMinutes - MORNING_START) / (MORNING_END - MORNING_START) * 100;
    }

    if (currentMinutes < schedule.afternoonStart) {
        return (currentMinutes - MORNING_END) / (schedule.afternoonStart - MORNING_END) * 100;
    }

    if (currentMinutes < schedule.dayEnd) {
        return (currentMinutes - schedule.afternoonStart) / (schedule.dayEnd - schedule.afternoonStart) * 100;
    }
...
function updateProgressPath() {
    if (!todayStrip || !todayProgressSvg || !todayProgressTrack || !todayProgressPath) return;

    var width = todayStrip.offsetWidth;
    var height = todayStrip.offsetHeight;
    if (!width || !height) return;
    if (width === todayProgressWidth && height === todayProgressHeight && todayProgressPathLength) return;

    var inset = 1;
    var radius = height / 2 - inset;
    var path = getRoundedRectPath(width, height, radius, inset);
